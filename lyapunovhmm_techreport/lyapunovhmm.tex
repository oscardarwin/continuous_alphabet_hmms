...
Thus, for any word $w$ starting with $a b a$ we have $L_n(w) = 2$.
...
A Markov chain $(Q,T)$ is naturally associated with its directed \emph{graph} $(Q,\{(q,r) \mid T_{q,r} > 0\})$, and so we may use graph concepts, such as strongly connected components (SCCs), in the context of a Markov chain.
...
By~$\EE_\pi$ we denote the expectation with respect to~$\PP_{\pi}$.
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
(The Dirac distributions on) $s_1$ and $s_2$ are distinguishable. Further, the increments $\ln L_{n + 1} - \ln L_n$ are independent and identically distributed (i.i.d.) and $0 > \EE_{s_2}[\ln L_{n+1} - \ln L_n] = p_2 \ln \frac{p_1}{p_2} + (1 - p_2)\ln \frac{1 - p_1}{1 - p_2} =: \ell$. Intuitively as $\ell$ gets more negative, the HMMs become more different.\footnote{In fact, $\ell$ is the \emph{KL-divergence} of the distributions $f_1, f_2$ where $f_i(a) = p_i$ and $f_i(b) = 1 - p_i$ for $i = 1, 2$.} Indeed, Wald~\cite{wald45} shows that the expected stopping time $\EE_{s_2}[N_{\alpha, \beta}]$ and $\ell$ are inversely proportional:
...
\end{equation}
This Wald formula cannot hold general for (multi-state) HMMs. The increments $\ln L_{n+1} - \ln L_n$ need not be independent and $\EE_{s_1}[\ln L_{n+1} - \ln L_n]$ can be different for different $n$. Further, $| \ln L_{n+1} - \ln L_n |$ can be unbounded; cf.~\cite[Example~6]{kief16}.
...
\end{definition}
We can restrict likelihood exponents to~$[-\infty, 0]$, as $\PP_{\pi_2}(\liexp > 0) = 0$ holds by \cref{convergenceLn}.1.
%We also have:
...
It follows that $\PP_{\pi_2}(E_0) = \frac12$ and $E_{\pi_2}[N_{\alpha, \beta} \mid E_0] = \infty$ and, thus, $E_{\pi_2}[N_{\alpha, \beta}] = \infty$.
\end{example}
...
A \emph{generalized Lyapunov system} is a triple $\S = (\M,\H,C)$ where $\M = (Q_1, \Sigma, \Psi_1)$ is matrix system and $\H = (Q_2, \Sigma, \Psi_2)$ is a strongly connected HMM and $C \subseteq Q_1 \times Q_2$ is a bottom SCC of $G_{\M, \H}$.
...
For sets $U, V \subseteq Q \times Q$ write $U \longrightarrow_{G_{\H,\H}} V$ denote that there are $u \in U$ and $v \in V$ such that $v$ is reachable from~$u$ in~$G_{\H,\H}$.
...
\[
\textstyle
\liexp \ =\ \frac23 \Big(\frac23 \ln \frac12 + \frac13 \ln \Big) + \frac13 \Big(\frac13 \ln 2 + \frac23 \ln \frac12\Big) \ = \ \frac13 \ln2 + \frac23 \ln \frac12 = - \frac13 \ln2\,.
\]
